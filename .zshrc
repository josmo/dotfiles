...
# Move a file from $HOME into the dotfiles repo ($DOTFILES, set by nix-darwin) at the
# same relative path, symlink it back, and stage it. Usage: dot-adopt ~/.config/foo/bar.toml
dot-adopt() {
  [[ -n "$DOTFILES" && -d "$DOTFILES/.git" ]] || { echo "DOTFILES is unset or not a git checkout" >&2; return 1; }
  local f rel
  for f in "$@"; do
    f="${f:a}"                       # absolute path, symlinks left alone
    rel="${f#$HOME/}"
    [[ "$rel" == /* ]]   && { echo "not under \$HOME: $f" >&2; return 1; }
    [[ "$rel" != .* ]]   && { echo "only dotfiles get linked: $rel" >&2; return 1; }
    [[ -L "$f" ]]        && { echo "already a symlink: $f" >&2; return 1; }
    [[ -f "$f" ]]        || { echo "not a regular file: $f" >&2; return 1; }
    grep -qiE 'token|secret|password|api_key|private' "$f" &&
      echo "warning: $rel may contain a credential, review before committing" >&2
    mkdir -p "$DOTFILES/${rel:h}"
    mv "$f" "$DOTFILES/$rel"
    ln -s "$DOTFILES/$rel" "$f"
    git -C "$DOTFILES" add "$rel"
    echo "adopted $rel"
  done
}
...
# Generate tenv completion at startup instead of tracking a generated file.
...
# BEGIN_AWS_SSO_CLI
# Generated by `aws-sso setup completions --install`, with the baked-in /nix/store
# path replaced by plain `aws-sso` so it survives nix updates.
...
aws-sso-profile() {
    ...
    if [ -n "$_sso" ]; then
        eval $(aws-sso ${=_args} -S "$_sso" eval -p "$_profile")
    else
        eval $(aws-sso ${=_args} eval -p "$_profile")
    fi
